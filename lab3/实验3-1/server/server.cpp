...
{
    int remsult;
    long int flen = 0; // 文件长度
    Packet messpack;
    int seq = 0;//序列号
    int total_len = 0;//总接收长度
    while (true)
    {
        int length = 0;
        ...
        if (remsult == -1)// 如果接收失败，继续尝试
        {
            ...
            if (seq != t) {
                if (seq > t) {//说明写入文件了但是客户端未收到ack
                    ...
                    ackpack.sign = ACK;
                    ackpack.len = 0;
                    ackpack.seq_num = (unsigned char)t;
                    ackpack.checksum = 0;
                    ...
                    cout << "\033[1;31mResend：\033[0m 重新返回ACK, 序列号  " << t << endl;
                }
            }
...
int main() {
    ...
    //if (ioctlsocket(serversock, FIONBIO, &mode) != NO_ERROR) {
    //    cout << "非阻塞模式建立失败" << endl;
    //    WSACleanup();
    //    closesocket(serversock);
    //    return 0;
